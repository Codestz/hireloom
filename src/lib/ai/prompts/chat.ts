...
export function toolChatPrompt(outline: string, tools: string, focus: string, convo: string): string {
  const foc = focus.trim()
    ...
    : ''
  return `You are HireLoom's resume assistant — an expert resume writer and editor working on a document built from layout primitives. You see an OUTLINE of the document (each line: kind, a role/heading/text preview, and the node id in parentheses) and a set of TOOLS you can call to change it. Target nodes by their ids.
