...
import { coverLetterPrompt, importStructurePrompt, tailorPrompt } from '../prompts/generators'

/** Stream an arbitrary generation prompt (used by the document generators). */
function streamPrompt(prompt: string, onChunk: (partial: string) => void): Promise<string> {
