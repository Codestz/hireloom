...
- **`src/lib/db`** — Dexie/IndexedDB. CRUD in `resumes.ts`, TanStack Query hooks in `queries.ts`. Includes backup/restore (export/import all résumés as JSON).
...
- **`service.ts`** — résumé-specific prompts (improve, summarize, cover letter, chat actions, tailor-to-JD, PDF→JSON structuring), all engine-agnostic.
...
- The Zod résumé schema is the source of truth; TS types are `z.infer`'d from it — never hand-written in parallel.
- Client-only modules (Dexie, pdf.js, the AI APIs) are imported dynamically so they stay out of the SSR/route bundle.
- No telemetry, no analytics, no user-data network calls. Private by default.
